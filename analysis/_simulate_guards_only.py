"""
Simulate variant: skip hour 14 + skip mins_to_close>=60 ONLY
(Keep current entry $0.30-$0.70, keep current vol threshold)
"""
import json, os
...
def variant(t):
    """Just two guards: skip hour 14, skip mins>=60"""
    return t["hour_utc"] != 14 and t["mins_to_close"] < 60
...
print(stats([t for t in trades if variant(t)],
            "PROPOSED: skip hr14 + skip close>=60m"))
print()

print("DELTA: trades removed by guards")
removed = [t for t in trades if not variant(t)]
print(stats(removed, "  removed"))
print()
...
for name, lo, hi in periods:
    ...
    print(stats([t for t in sub if variant(t)], "PROPOSED (guards only)"))
    print()

# Daily breakdown
print("=" * 100)
print("PROPOSED FILTER (guards only)  -  DAILY BREAKDOWN")
print("=" * 100)
...
print()
print("=" * 100)
print("WALK-FORWARD: PROPOSED filter (guards only) on held-out test windows")
print("=" * 100)
folds = [
